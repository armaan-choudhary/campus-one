...
export const AgentQueueView: React.FC = () => {
  ...
  const quickTemplates = [
    'Issue resolved in institutional Active Directory / ERP. Student access restored.',
    'Work order dispatched to campus facility engineering. SLA estimated: 4 hours.',
    'Bursar payment ledger verified against bank UTR. Hold cleared on student portal.',
  ];
    ...
    <div className="flex-1 overflow-y-auto p-6 max-w-6xl mx-auto space-y-6 transition-all">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div className="p-5 rounded-3xl bg-[var(--surface-1)] border border-[var(--border-subtle)] space-y-1.5 shadow-xs">
          ...
          <div className="text-2xl font-bold text-[var(--foreground)] tracking-tight">{tickets.length}</div>
          <span className="text-[11px] text-[var(--accent)] font-medium">All departments</span>
        </div>
        <div className="p-5 rounded-3xl bg-[var(--surface-1)] border border-[var(--border-subtle)] space-y-1.5 shadow-xs">
      ...
      {/* Ticket Cards Grid with Generous Spacing */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {filteredTickets.length === 0 ? (
          ...
          filteredTickets.map((ticket) => (
            <div
              key={ticket.ticketId}
              className="p-7 rounded-3xl bg-[var(--surface-1)] border border-[var(--border-subtle)] hover:border-[var(--border-medium)] transition-all flex flex-col justify-between gap-6 shadow-xs hover:shadow-md"
            >
              ...
              {/* Bottom Actions with Generous Spacing */}
      ...
      {/* Ticket Inspection Modal with Generous Vertical Space */}
      {selectedTicket && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xs p-4 sm:p-6 animate-in fade-in duration-200">
          <div className="w-full max-w-xl bg-[var(--surface-1)] border border-[var(--border-subtle)] rounded-3xl p-8 shadow-2xl space-y-6 animate-in zoom-in-95 duration-200 max-h-[90vh] overflow-y-auto">
            {/* Modal Header */}
            ...
            <div className="space-y-4 text-xs">
              ...
              <div>
                <span className="text-[10px] text-[var(--text-tertiary)] uppercase font-mono block mb-1.5">
                  Conversational Transcript Preserved
                </span>
                <div className="p-4 rounded-2xl bg-[var(--surface-2)] text-[var(--foreground)] leading-relaxed space-y-2.5 border border-[var(--border-subtle)]">
                  <p className="text-xs leading-relaxed">{selectedTicket.preview}</p>
                ...
                <div className="space-y-2 pt-2">
                  ...
                  <div className="pt-2">
                    ...
                    <textarea
                      ...
                      placeholder="Add custom notes or use template above..."
                      rows={2}
